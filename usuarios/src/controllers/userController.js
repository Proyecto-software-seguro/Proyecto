...
exports.registerUser = async (req, res) => {
    const { nombre, email, password, rol = 'cliente', sueldo, carga_familiar, direccion } = req.body; // Nuevos campos agregados
    ...
    try {
        ...
        const newUser = await createUser(nombre, email, hashedPassword, roleId, sueldo, carga_familiar, direccion);
...
exports.loginUser = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(
            { id: user.id, email: user.email, rol: user.rol_nombre, sueldo: user.sueldo, carga_familiar: user.carga_familiar, direccion: user.direccion }, // Agregamos nuevos datos en el token
        ...
        res.status(200).json({
            ...
            token,
            user: { id: user.id, nombre: user.nombre, email: user.email, rol: user.rol_nombre, sueldo: user.sueldo, carga_familiar: user.carga_familiar, direccion: user.direccion }
        });
    } catch (error) {
...
exports.getUserProfile = async (req, res) => {
    try {
        ...
        const query = `
            SELECT u.id, u.nombre, u.email, r.nombre AS rol, u.sueldo, u.carga_familiar, u.direccion
            FROM usuarios u
